...
# Terminal 2: Lifecycle operations and reset dead reckoning and launch bringup after it is done
tmux new-window -t $SESSION_NAME -n 'lifecycle and bringup'
# We wait slightly to ensure the DVL node is spun up before configuring
tmux send-keys -t $SESSION_NAME:1 "$(run_ros_cmd 'sleep 5 && ros2 lifecycle set /dvl_a50 configure && ros2 lifecycle set /dvl_a50 activate && ros2 service call /reset_dead_reckoning std_srvs/Trigger {} && ros2 launch bringup bringup.launch.py')" C-m
...
tmux send-keys -t $SESSION_NAME:2 "$(run_ros_cmd 'sleep 25 && ros2 launch planner planner.launch.py sim:=false')" C-m

# Terminal 4: Mission Configuration
tmux new-window -t $SESSION_NAME -n 'mission'
# We wait 30 seconds for planner and bringup to register their topics
tmux send-keys -t $SESSION_NAME:3 "$(run_ros_cmd 'sleep 30 && ros2 topic pub --once /mission_selector std_msgs/msg/Int32 "{data: 6}"')" C-m
