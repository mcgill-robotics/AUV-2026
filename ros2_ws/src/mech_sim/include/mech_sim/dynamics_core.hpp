...
// State x = [  u v w             p q r        qw qx qy qz       px py pz ]^T
...
private:
  ...
  static Mat3 R_from_q(const Vec4& q);
  ...
  Vec6 gravity_buoyancy_wrench_body(const Mat3& R_body_to_odom, const double depth) const;


  /// Compute damping wrench in body frame.
  Vec6 damping_wrench_body(const Vec3& v_body, const Vec3& w_body) const;

  /// Compute body accelerations: nudot = M^{-1}( tau - D*nu - gb )
  /// where nu = [u v w p q r]^T, tau_body = [Fx Fy Fz Mx My Mz]^T,
  ...
  static Vec4 q_dot(const Vec4& q, const Vec3& w_body);


  // Compute position derivative
  static Vec3 p_odom_dot(const Vec4& q, const Vec3& v_body);
