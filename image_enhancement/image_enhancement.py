...
class EnhancementAlgorithm(ABC):
    """Abstract Base Class containing various
    image enhancement algorithms for underwater images."""
...
class ImageEnhancer:
    ...
    By default, applies DPC and CLAHE sequentially.
...
class DCPEnhancement(EnhancementAlgorithm):
    ...
    def apply_algorithm(self, image: np.ndarray) -> np.ndarray:
        ...
        indices = np.argpartition(flat_dark, -int(0.001 * flat_dark.size))[
            -int(0.001 * flat_dark.size) :
        ]
    ...
    def get_dark_channel(self, img):
        ...
        kernel = cv2.getStructuringElement(
            cv2.MORPH_RECT, (self.window_size, self.window_size)
        )
